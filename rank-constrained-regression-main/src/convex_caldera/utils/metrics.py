...
def compute_bits_per_parameter(
    ...
    """
    Compute bits-per-parameter accounting for low-rank structure.
    
    Args:
        num_params: Total number of parameters
        avg_bit_width: Average bit-width for quantized component
        effective_rank: Effective rank of low-rank component
    ...
    # Low-rank: rank * (m + n) * rank_bits
    # Quantized: m * n * avg_bit_width
    # Total bits = rank * (m + n) * rank_bits + m * n * avg_bit_width
    # Bits per parameter = Total bits / (m * n)
    
    # Approximate: assume square matrix
    sqrt_params = np.sqrt(num_params)
    rank_contribution = effective_rank * 2 * sqrt_params * rank_bits
    quant_contribution = num_params * avg_bit_width
    total_bits = rank_contribution + quant_contribution
    
    return total_bits / num_params
...
def evaluate_compression(
    ...
    bits_per_param = compute_bits_per_parameter(
        num_params, avg_bit_width, effective_rank, rank_bits
    )
