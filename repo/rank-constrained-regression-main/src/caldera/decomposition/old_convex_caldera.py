...
def compute_hessian_and_sensitivities(
    ...
    c = float(torch.var(W).item() * 0.1)  # Scaling factor，显式转为 Python float
...
def solve_convex_optimization(
    ...
    c_safe = cp.reshape(cp.maximum(c, 1e-8), (1,))
    # 直接用 reshape 确保第三个参数的形状
    constraints.append(cp.constraints.ExpCone(q, c_safe, cp.reshape(-params.k * b * c_safe, (1,))))
    ...
    # 尝试使用间接线性解算器并放宽容差以避免 LDL 因子分解的崩溃
            problem.solve(
                solver=cp.SCS,
                verbose=params.solver_verbose,
                eps=max(params.solver_tol, 1e-4),
                use_indirect=True,
                max_iters=10000
            )
