...
_BITWIDTHS = [2, 4, 8, 16]
_QUANTIZER_METHODS = ["uniform", "nf4", "nf2", "bbint4", "bbint2"]
...
class LowMemoryQuantizer(AbstractQuantizer):
    ...
    def _quantize_bbint2(self, weight_blocks: torch.Tensor, epsilon: float = 1e-8, log_file: str = "outlier_log.csv"):
            """Quantizes using bitsandbytes-style INT2 quantization."""
            device = weight_blocks.device
            
            # Detect outliers (same as bbint4)
            weight_mean = weight_blocks.mean(dim=1, keepdim=True)
            weight_std = weight_blocks.std(dim=1, keepdim=True)
            weight_std = torch.maximum(weight_std, torch.tensor([epsilon], device=device))
            outlier_mask = (weight_blocks - weight_mean).abs() > (6.0 * weight_std)
            ...
            if not os.path.exists(log_file):
                with open(log_file, mode='w', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(["Call_ID", "Num_Outliers"])

            with open(log_file, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([id(self), num_outliers])
            ...
            block_min = weight_blocks.min(dim=1, keepdim=True)[0]
            block_max = weight_blocks.max(dim=1, keepdim=True)[0]
            scales = (block_max - block_min) / 3  # Changed from 15 to 3 for 2-bit quantization
            scales = torch.maximum(scales, torch.tensor([epsilon], device=device))
            
            # Quantize to 2-bit range (0-3 instead of 0-15)
            weight_scaled = (weight_blocks - block_min) / scales
            weight_quant = torch.clamp(torch.round(weight_scaled), 0, 3)
            
            # Pack four 2-bit values into one byte
            weight_packed = (weight_quant[:, ::4] * 64 + 
                            weight_quant[:, 1::4] * 16 +
                            weight_quant[:, 2::4] * 4 +
                            weight_quant[:, 3::4]).to(torch.uint8)
            
            return weight_packed, (block_min, scales, outlier_values, outlier_indices)
    def _dequantize_bbint2(self, weight_packed: torch.Tensor, params):
            """Dequantizes bitsandbytes-style INT2 quantized values."""
            block_min, scales, outlier_values, outlier_indices = params
            device = weight_packed.device
            
            # Unpack bytes into 2-bit values
            weight_unpacked = torch.zeros(weight_packed.shape[0], weight_packed.shape[1]*4, 
                                        dtype=torch.float32, device=device)
            weight_unpacked[:, ::4] = weight_packed // 64
            weight_unpacked[:, 1::4] = (weight_packed % 64) // 16
            weight_unpacked[:, 2::4] = (weight_packed % 16) // 4
            weight_unpacked[:, 3::4] = weight_packed % 4
    ...
    def quantize_block(self, weight: torch.Tensor, epsilon: float = 1e-8):
        ...
        elif self.method == "bbint2":
            weight_quant, weight_params = self._quantize_bbint2(weight_blocks, epsilon)    
        else:
    ...
    def dequantize_block(self, weight_quant: torch.Tensor, weight_params, weight_shape: List[int]):
        ...
        if self.method == "uniform":
            ...
            weight_blocks = self._dequantize_bbint2(weight_quant, weight_params)    
        else:
